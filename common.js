...
function getCodeSamples(data) {
    ...
    if (op && op["x-code-samples"]) {
        for (var c in op["x-code-samples"]) {
            ...
            s += generateCodeSnippet(lang, sample.source, true);
        }
    }
    else {
        ...
        const codeSamples = data.header.language_tabs
            .map(tab => {
                ...
                if(lang==`python`){
                    return (sample && generateCodeSnippet(lowerCaseLanguage, sample, false, true)) || '';
                } else if(lang=='go'){
                    return (sample && generateCodeSnippet(lowerCaseLanguage, sample, true, false)) || '';
                } else {
...
function generateCodeSnippet(lang, code, first, last) {
